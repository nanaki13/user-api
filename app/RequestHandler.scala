...
import com.mohiva.play.silhouette.api.{Authenticator, Authorization, Silhouette}
...
class SecuredFilter @Inject() (silhouette: Silhouette[module.JWTEnv], bodyParsers: PlayBodyParsers,userController: UserController)(implicit val mat: Materializer)  extends Filter {
  import play.api.mvc._
  override def apply(next: RequestHeader => Future[Result])(
    ...
    val action = silhouette.UserAwareAction.async(bodyParsers.empty) { r =>
      request.path match {
        case "/admin" if r.identity.isEmpty => Future.successful(userController.Unauthorized)
        case _ => next(request)
      }
    }
